...
def fully_connected(value, output_shape, name='fully_coonnected', with_w=False):
...
def deconv2d(value, output_shape, k_h=5, k_w=5, strides=[1, 2, 2, 1],
    ...
    with tf.variable_scope(name):
        ...
        deconv = tf.nn.conv2d_transpose(value, weights, output_shape, strides)
...
def conv2d(value, output_dim, k_h=5, k_w=5,
    ...
    with tf.variable_scope(name):
        ...
        conv = tf.nn.conv2d(value, weights, strides, 'SAME')
...
def conv_cond_concat(value, cond, name='concat'):
    ...
    with tf.variable_scope(name):
        return tf.concat([value, cond * tf.ones(value_shape[0:3] + cond_shapes[3:])], 3)
    ...
    with tf.variable_scope(name):
        ...
        if is_train:
            ...
            with tf.control_dependencies([assign_mean, assign_variance]):
                return tf.nn.batch_normalization(value, mean, variance, bata, gamma, 1e-5)
...
def descriminator(image, reuse=False, name='descriminator'):
    with tf.name_scope(name):
        if reuse:
...
def train():
    ...
    G = generator(z)
    D,D_logits = descriminator(images)
    samples = sampler(z)
    D_,D_logits_ = descriminator(G,reuse=True)
