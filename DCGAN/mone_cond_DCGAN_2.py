...
def batch_norm(value, is_train=True, name='batch_norm',
    ...
    return batch_norm(value, decay=momentum, epsilon=epsilon, scale=True,
...
def generator(z, is_train=True, name='generator'):
    with tf.name_scope(name):
        ...
        s2, s4, s8, s16 = int(s2), int(s4), int(s8), int(s16)
...
def read_and_decode(filename_queue):
    ...
    image = tf.cast(image, tf.float32)  # tf.cast(x,dtype,name=None),将x的数据格式转换成dtype。
    image = image / 255.0
...
def train():
    with tf.variable_scope("train"):
        ...
        train_dir = CURRENT_DIR + '/logs_without_cd_h1_bnondition' \
                                  '_2/'
        ...
        d_loss_real = tf.reduce_mean(tf.scalar_mul(-1,D_logits)) #tf.scalar_mul(scalar,x) 求x的scalar倍
        ...
        d_optim = tf.train.RMSPropOptimizer(LR) \
            .minimize(d_loss, var_list=d_vars, global_step=global_step)
        g_optim = tf.train.RMSPropOptimizer(LR) \
            .minimize(g_loss, var_list=g_vars, global_step=global_step)
        clip_d_op = [var.assign(tf.clip_by_value(       #tf.clip_by_value(A, min, max)：输入一个张量A，把A中的每一个元素的值都压缩在min和max之间。小于min的让它等于min，大于max的元素的值等于max。
            var,CLIP[0],CLIP[1])) for var in d_vars]
        ...
        for epoch in range(EPOCH):
            ...
            for idx in range(start, batch_idxs):
                ...
                if idx % 100 == 0:
                    ...
                    samples_path = CURRENT_DIR + '/samples_without_condition_2/'
                ...
                if (idx % 256 == 0) or (idx + 1 == batch_idxs):
...
def evaluate():
    ...
    checkpoint_dir = CURRENT_DIR + '//logs_without_cd_h1_bnondition_2_2/'

    z = tf.placeholder(tf.float32, [None, Z_DIM], name='z')

    with tf.variable_scope('train/test'):
